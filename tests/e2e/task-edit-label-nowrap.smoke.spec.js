...
// 親 .widget-control 側へ font-size / font-weight を複製する旧実装は、ラベルの
// 文字サイズだけを変更する将来の変更で ch の基準がずれる二重定義の危険があった
// ため廃止した）。
//
...
//   2. 各行でラベル右端が同じ行の select 左端を越えないこと（重なりの検出）。
...
test('サイドバー: タスク編集パネルの項目名ラベルが1行に収まり select 列の左端が揃う（issue #470）', async () => {
  ...
  try {
    ...
    for (const m of measurements) {
      ...
      // ─── (2) ラベル右端が同じ行の select 左端を越えない（重なりの検出） ───
